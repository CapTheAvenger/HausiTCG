...
def load_settings() -> Dict[str, Any]:
    """Load settings from city_league_archetype_settings.json, or create it with defaults if it doesn't exist."""
    app_path = get_app_path()
    settings_path = os.path.join(app_path, 'city_league_archetype_settings.json')
    ...
    if not os.path.exists(settings_path) and os.path.basename(app_path) == 'dist':
        ...
        parent_settings_path = os.path.join(parent_path, 'city_league_archetype_settings.json')
    ...
    if os.path.exists(settings_path):
        try:
            ...
            print(f"Error loading city_league_archetype_settings.json: {e}")
        ...
        print(f"Created default city_league_archetype_settings.json at {settings_path}")
        return DEFAULT_SETTINGS.copy()
...
def get_tournaments_in_date_range(region: str, start_date: datetime, end_date: datetime, delay: float = 1.5) -> List[Dict[str, str]]:
    ...
    for tournament in parser.tournaments:
        ...
        if tournament_date and start_date <= tournament_date <= end_date:
            ...
            print(f"  [OK] {tournament['date_str']} - {tournament.get('prefecture', 'N/A')} - {tournament.get('shop', 'N/A')} (ID: {tournament['tournament_id']})")
...
def create_comparison_report(old_data: List[Dict[str, str]], new_data: List[Dict[str, str]], output_file: str):
    ...
    comparison_html_local = os.path.join(app_path, output_file.replace('.csv', '_comparison.html'))
